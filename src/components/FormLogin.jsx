...
const FormLogin = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if(!userExist) setErrors({...errors , email: true})
        if(Object.values(errors).indexOf(false) === -1) e.target.reset()
    }
    ...
    return (
        ...
        onSubmit={handleSubmit} className="flex py-10 flex-col items-center w-full md:w-2/5 ">
            ...
            {
                errors.email &&
                <span className="text-red-400 mb-2 text-sm font-semibold">email yang dimasukkan tidak terdaftar</span>
            }
            ...
            {
                errors.password &&
                <span className="text-red-400 mb-2 text-sm font-semibold">password yang dimasukkan salah</span>
            }
            <div className="w-3/4 flex gap-4 justify-center mt-6">
                <button type="button" className="bg-gray-400 hover:bg-gray-500 transition duration-200 text-white py-2 w-2/3 font-semibold rounded-md " onClick={()=>dispatch({ type: "CHANGE_TO_REGISTER" })}>Buat Akun?</button>
